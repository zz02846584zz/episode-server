...
class TestPlugin extends command_core_1.BasePlugin {
    constructor() {
        super(...arguments);
        this.commands = {
            test: {
    ...
    async run() {
        ...
        const isTs = this.options.ts || (0, fs_1.existsSync)((0, path_1.join)(cwd, 'tsconfig.json'));
        let testFiles = [];
        if (this.options.f) {
        ...
        if (this.options.mocha) {
            try {
                ...
                this.core.cli.log('');
                this.core.cli.error('Using mocha test need deps ', this.options.cov ? 'nyc' : 'mocha');
                throw e;
            }
        }
        else {
            const jestModuleDir = (0, command_core_1.findNpmModuleByResolve)(process.cwd(), 'jest');
            if (!jestModuleDir) {
                const errorMsg = [
                    '!!!',
            ...
            binFile = (0, path_1.join)(jestModuleDir, './bin/jest');
        ...
        this.runTypeCheck();
        return (0, command_core_1.forkNode)(binFile, args, opt);
    }
    async formatJestTestArgs(isTs, testFiles) {
        ...
        args.push(`--config=${(0, path_1.join)(__dirname, `../config/${isTs ? 'jest.ts.js' : 'jest.js'}`)}`);
        return args;
    }
    async formatMochaTestArgs(isTs, testFiles) {
        ...
        const setupFile = (0, path_1.join)(process.cwd(), `test/.setup.${isTs ? 'ts' : 'js'}`);
        if ((0, fs_1.existsSync)(setupFile)) {
    ...
    runTypeCheck() {
        ...
        (0, typeCheck_1.typeCheck)(this.core.cwd);
    }
}
